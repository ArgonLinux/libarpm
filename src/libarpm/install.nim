import
  std/[os, options, strutils, tempfiles, posix],
  ./[io, storage, package_list, package, helpers, dependencies],
  zippy/ziparchives
...
proc install*(
  ...
  if not data.isSome:
    error "Package \"" & package.name &
      "\" was not found in the binary package repository."
    error(
      "If this package is real, then this means that the package list repository and the binary package repository are de-synchronized. Please report this!",
      true
    )
  ...
  let dirs = getFirstDirectory(dir / "files")
  ...
  for file in package.files:
    ...
    if not fileExists(prefix / real):
      error(
        "Could not find file: \"" & real & "\" inside package data! Please report this!",
        true,
      )
    ...
    root:
      ...
      if chmod(
        file,
        (
          S_IRUSR or S_IWUSR or S_IXUSR or S_IRGRP or S_IWGRP or S_IXGRP or S_IROTH or
          S_IWOTH or S_IXOTH
        ).Mode,
      ) != 0:
